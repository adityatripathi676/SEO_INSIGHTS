...
You will receive scraping data containing a full site audit (on-page, technical, lighthouse performance, authority) AND the live Google SERP organic rankings for the target keyword.
...
Competitive Landscape
	•	Analyze the LIVE ORGANIC SERP RANKINGS provided in the data.
	•	Extract the top ranking domains as competitors and include their exact SERP position.
	•	Calculate strength scores (0.0-10.0) based on their ranking position and snippet quality.
	•	Identify who holds the #1 spot and why their snippet/meta description might be winning.
	•	Compare their on-page metrics and authority if available.
...
Recommendations
	•	Provide a clear, actionable 'title' and a detailed 'description' for each recommendation.
	•	Include specific 'implementation_steps' (array of strings) detailing how to execute it.
...
OUTPUT REQUIREMENTS
	...
	•	Numbers → 0
	•	Summary → MUST BE POPULATED. Calculate an estimated overall_score (0-100) based on whatever data is available. Provide generic fallback text for strengths and issues if necessary.
	•	Unknown fields → null
